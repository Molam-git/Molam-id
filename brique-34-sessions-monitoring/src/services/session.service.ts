 ...
 * Handles session lifecycle, heartbeat, revocation, and anomaly detection
 */
import db from '../db';
...
/**
 * Get session policies from database
 */
...
/**
 * List active sessions for a user (self-service)
 */
...
/**
 * Heartbeat to extend session expiration
 */
export async function heartbeat(sessionId: string, ip?: string, ua?: string) {
  ...
  const sess = await db.oneOrNone(
    'SELECT * FROM molam_sessions WHERE id=$1 AND is_active=TRUE',
    [sessionId]
  );
...
export async function revokeOne(
  ...
  await db.none(
    `INSERT INTO molam_session_activity(session_id, event, meta)
     VALUES ($1,'REVOKE', $2::jsonb)`,
    [sessionId, JSON.stringify({ by: actorUserId })]
  );
 ...
 */
export async function revokeAllButCurrent(
  actorUserId: string,
  userId: string,
  ...
  const sessions = await db.any(
    `SELECT id FROM molam_sessions WHERE user_id=$1 AND is_active=TRUE AND id<>$2`,
    [userId, currentSessionId]
  );

  for (const s of sessions) {
    await db.none(
      `UPDATE molam_sessions SET is_active=FALSE, revoked_at=NOW(), revocation_reason='bulk_user_revoke' WHERE id=$1`,
      [s.id]
    );
    await db.none(
      `INSERT INTO molam_session_activity(session_id, event, meta)
       VALUES ($1,'REVOKE', '{"bulk": true}')`,
      [s.id]
    );
 ...
 */
export async function adminRevokeByFilter(
  actorUserId: string,
  ...
  if (filter.deviceId) {
    args.push(filter.deviceId);
    where.push(`device_id=$${args.length}`);
  }
  if (filter.olderThanMinutes) {
    args.push(filter.olderThanMinutes);
    where.push(`last_seen_at < NOW() - ($${args.length}::text||' minutes')::interval`);
  }
  ...
  for (const v of victims) {
    await db.none(
      `UPDATE molam_sessions SET is_active=FALSE, revoked_at=NOW(), revocation_reason='admin_filter' WHERE id=$1`,
      [v.id]
    );
    await db.none(
      `INSERT INTO molam_session_activity(session_id, event, meta) VALUES ($1,'REVOKE','{"admin_filter":true}')`,
      [v.id]
    );
...
/**
 * Detect anomalies on login
 */
export async function detectAnomaliesOnLogin(
  sessionId: string,
  prevGeo: any,
  newGeo: any,
  fpScore: number
  ...
  // Impossible travel
  if (prevGeo && newGeo) {
    const speed = estimateSpeedKmH(prevGeo, newGeo);
    const th = policies.impossible_travel?.kmh_threshold ?? 900;
    if (speed > th) {
      await markAnomaly(sessionId, 'IMPOSSIBLE_TRAVEL', 'high', {
        speed,
        threshold: th,
        prevGeo,
        newGeo,
...
/**
 * Mark an anomaly and publish event
 */
async function markAnomaly(sessionId: string, kind: string, severity: string, details: any) {
  await db.tx(async (t) => {
    const a = await t.one(
      ...
      [sessionId, kind, severity, details]
    );
    await t.none(
      `INSERT INTO molam_session_activity(session_id, event, meta) VALUES ($1,'ANOMALY',$2::jsonb)`,
...
/**
 * Estimate travel speed between two geolocations
 */
export function estimateSpeedKmH(
  ...
  const dtHours = Math.max((b.at - a.at) / 3600000, 0.0003); // avoid division by zero
  return d / dtHours;
}

/**
 * WORM audit for critical actions
 */
async function wormAudit(
  actor: string | null,
  action: string,
  sessionId: any,
  payload: any,
  ...
  const secret = process.env.WORM_SIGNING_SECRET || 'dev-secret-change-in-production';
...
/**
 * Update session policies (admin)
 */
export async function updateSessionPolicies(actorUserId: string, policies: any, ip?: string, ua?: string) {
  for (const [key, value] of Object.entries(policies)) {
    await db.none(
      `INSERT INTO id_session_policies(key, value, updated_at)
       VALUES ($1, $2, NOW())
  ...
  await wormAudit(actorUserId, 'POLICY_UPDATE', null, { policies }, ip, ua);
  return { ok: true };
}
